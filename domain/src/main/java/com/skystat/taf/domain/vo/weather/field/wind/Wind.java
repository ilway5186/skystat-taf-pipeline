...
public record Wind(
	...
	public Wind {
		...
		if (speed != null && unit.toStandardUnitValue(speed) > WIND_STANDARD_LIMIT) {
			throw new IllegalArgumentException(String.format(
				"Wind speed can't exceed %sKT. speed: %s, unit: %s",
				WIND_STANDARD_LIMIT, speed, unit
			));
		}

		if (gust != null && unit.toStandardUnitValue(gust) > WIND_STANDARD_LIMIT) {
			throw new IllegalArgumentException(String.format(
				"Wind gust can't exceed %sKT. speed: %s, unit: %s",
				WIND_STANDARD_LIMIT, gust, unit
			));
		}
	}
