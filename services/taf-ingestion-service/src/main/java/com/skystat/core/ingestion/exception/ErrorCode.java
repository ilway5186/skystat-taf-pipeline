...
public enum ErrorCode {
  ...
  DUPLICATE_RESOURCE(HttpStatus.CONFLICT, "DUPLICATE_RESOURCE", "The resource already exists."), // 이미 존재하는 리소스입니다.

  // 422 - Business rule 계열
  BUSINESS_RULE_VIOLATION(HttpStatus.UNPROCESSABLE_CONTENT, "BUSINESS_RULE_VIOLATION", "A business rule was violated."), // 비즈니스 규칙을 위반했습니다.
  INSUFFICIENT_BALANCE(HttpStatus.UNPROCESSABLE_CONTENT, "INSUFFICIENT_BALANCE", "Insufficient balance."), // 잔액이 부족합니다.

  // 502/503 - External (외부 시스템 연동관련 오류)
  EXTERNAL_SERVICE_ERROR(HttpStatus.BAD_GATEWAY, "EXTERNAL_SERVICE_ERROR", "An error occurred while connecting to an external system."), // 외부 시스템 연동 중 오류가 발생했습니다.
  EXTERNAL_SERVICE_UNAVAILABLE(HttpStatus.SERVICE_UNAVAILABLE, "EXTERNAL_SERVICE_UNAVAILABLE", "The external system is temporarily unstable."), // 외부 시스템이 일시적으로 불안정합니다.

  // 500 - Internal (서버 오류 계열)
  INTERNAL_ERROR(HttpStatus.INTERNAL_SERVER_ERROR, "INTERNAL_ERROR", "An internal server error occurred."); // 서버 오류가 발생했습니다.
